...
function get_site_url($full)
{
    ...
    if ($o !== false) {
        $uriroot = substr($uriroot, 0, $o);
    }
    $docroot = realpath($_SERVER['DOCUMENT_ROOT']);
    $o = strpos($uriroot, $docroot);
    if ($o !== false) {
        $uriroot = substr($uriroot, $o + strlen($docroot));
    }
    if (!$full) {
        return $uriroot;
    }
...
function safe_html_string($s, $paragraph = false)
{
    ...
    while (preg_match($s1, $s)) {
        $s = preg_replace($s1, $r1, $s);
    }
    while (preg_match($s2, $s)) {
        $s = preg_replace($s2, $r2, $s);
    }
    while (preg_match($s3, $s)) {
        $s = preg_replace($s3, $r3, $s);
    }
    while (preg_match($s4, $s)) {
        $s = preg_replace($s4, $r4, $s);
    }
    if ($paragraph) {
